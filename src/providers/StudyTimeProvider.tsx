...
export const useStudyTime = () => {
  ...
  if (!context) {
    return {
      ...
      refreshStudyTime: async () => {},
    };
  }
  return context;
};

export const StudyTimeProvider = ({ children }: { children: React.ReactNode }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    // Track user activity events globally
  ...
  return (
    <StudyTimeContext.Provider value={{
      ...studyTimeData,
      formatTime,
