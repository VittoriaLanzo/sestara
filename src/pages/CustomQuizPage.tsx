...
export type { CustomQuizQuestion, CustomQuiz, SavedQuiz, QuizGroup } from "@/hooks/useCustomQuizzes";
...
const CustomQuizPage = () => {
  ...
  const handleStartQuiz = (quiz: CustomQuiz, mode: 'timer' | 'track', minutes: number, quizId?: string) => {
    setActiveQuiz(quiz);
  ...
  const handleSaveQuiz = (quiz: CustomQuiz, groupId?: string) => {
    const groupName = quizGroups.find(g => g.id === (groupId || 'default'))?.name || 'General';
    saveQuizMutation.mutate({ quiz, groupId, groupName });
  };
  ...
  if (activeQuiz && showResults) {
    return (
      <CustomQuizResults
        ...
        onSaveQuiz={activeQuizId ? undefined : () => {
              ...
              <TabsContent value="paste" className="mt-6">
                ...
                >
                  <PasteJsonSection 
                    ...
                    onStartQuiz={(quiz, mode, minutes) => handleStartQuiz(quiz, mode, minutes)}
                  />
                </motion.div>
              </TabsContent>
